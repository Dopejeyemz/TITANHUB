--[=[
    ...
    FLASH HUB PRIV v1.0 — USE AT YOUR OWN RISK
]=]--


-- patch collectgarbage for restrictive executors
pcall(function()
    local _cg = collectgarbage
    collectgarbage = function(opt, ...) return _cg(opt or "count", ...) end
end)
...
local FileName = "FlashHub_v1.json" 
local DefaultConfig = {
    Positions = {
...
pcall(function()
    local _tm = {
        Red={Color3.fromRGB(255,50,50),Color3.fromRGB(200,30,30)},
        Blue={Color3.fromRGB(50,130,255),Color3.fromRGB(30,90,200)},
        Pink={Color3.fromRGB(255,80,180),Color3.fromRGB(200,40,140)},
        Green={Color3.fromRGB(50,210,100),Color3.fromRGB(30,160,70)},
        Orange={Color3.fromRGB(255,140,30),Color3.fromRGB(200,100,10)},
        Gold={Color3.fromRGB(255,200,40),Color3.fromRGB(200,150,10)},
        White={Color3.fromRGB(240,240,240),Color3.fromRGB(180,180,180)},
        Black={Color3.fromRGB(80,80,80),Color3.fromRGB(40,40,40)},
    }
    local _t = _tm[Config and Config.ThemeColor or "Red"]
    if _t then Theme.Accent1=_t[1]; Theme.Accent2=_t[2] end
end)

-- repaintAccents: repaints all tagged accent objects across all GUIs
do
    local function repaintAccents()
        local guiNames = {
            "SettingsUI","AutoStealUI","StealSpeedUI","FlashHubAdminPanel",
            "XiInvisPanel","FlashHubMobileControls","XiStatusHUD","XiJobJoiner","FlashHub_AutoBuy",
        }
        local function walk(gui)
            for _, obj in ipairs(gui:GetDescendants()) do
                pcall(function()
                    local tag = obj:GetAttribute("_acc")
                    if not tag then return end
                    if     tag=="A1bg"  then obj.BackgroundColor3 = Theme.Accent1
                    elseif tag=="A2bg"  then obj.BackgroundColor3 = Theme.Accent2
                    elseif tag=="A1tx"  then obj.TextColor3       = Theme.Accent1
                    elseif tag=="A1col" then obj.Color            = Theme.Accent1
                    elseif tag=="A2col" then obj.Color            = Theme.Accent2
                    end
                end)
            end
        end
        for _, name in ipairs(guiNames) do
            ...
            if g then pcall(walk, g) end
        end
        pcall(function()
            local cg = game:GetService("CoreGui"):FindFirstChild("FlashHub_AutoBuy")
            if cg then walk(cg) end
        end)
    end
    _G._repaintAccents = repaintAccents
end
local PRIORITY_LIST = {
   "Strawberry Elephant",
   "Meowl",
...
-- Load saved priority list from config (overrides default if user has saved one)
if Config.PriorityList and type(Config.PriorityList) == "table" and #Config.PriorityList > 0 then
    PRIORITY_LIST = Config.PriorityList
end
...
local function ShowNotification(title, text)
    local existing = PlayerGui:FindFirstChild("FlashHubNotif")
    if existing then existing:Destroy() end

    local sg = Instance.new("ScreenGui", PlayerGui)
    sg.Name = "FlashHubNotif"; sg.ResetOnSpawn = false
    ...
    stroke.Thickness = 1; stroke.Color = Theme.Accent2; pcall(function() stroke:SetAttribute("_acc","A2col") end); stroke.Transparency = 1

    local bar = Instance.new("Frame", f)
    bar.Size = UDim2.new(0, 3, 1, -12); bar.Position = UDim2.new(0, 5, 0, 6)
    bar.BackgroundColor3 = Theme.Accent1; pcall(function() bar:SetAttribute("_acc","A1bg") end); bar.BorderSizePixel = 0
    ...
    t1.TextColor3 = Theme.Accent1; pcall(function() t1:SetAttribute("_acc","A1tx") end); t1.TextXAlignment = Enum.TextXAlignment.Left
...
task.spawn(function()
    ...
    local instantStealReady = true
    local instantStealDidInit = true
    ...
    mainStroke.Color = Theme.Accent2; pcall(function() mainStroke:SetAttribute("_acc","A2col") end); mainStroke.Thickness = 1.5; mainStroke.Transparency = 0.4
    ...
    if IS_MOBILE then
        ...
        menuToggleBtn.BackgroundColor3 = Theme.Accent1; pcall(function() menuToggleBtn:SetAttribute("_acc","A1bg") end)
        ...
        if SharedState.ListNeedsRedraw then
            ...
            if allPets and #allPets > 0 then
                for i = 1, #allPets do
                    ...
                    bStroke.Color = Theme.Accent1; pcall(function() bStroke:SetAttribute("_acc","A1col") end); bStroke.Thickness = 1; bStroke.Transparency = 1
    ...
    customizePriorityBtn.BackgroundColor3 = Theme.Accent2; pcall(function() customizePriorityBtn:SetAttribute("_acc","A2bg") end)
    ...
    _G._firePrompt = function(p)
        if not p or not p.Parent then return end
        buildStealCallbacks(p)
        local d = InternalStealCache[p]
        if d then
            local od = p.HoldDuration; p.HoldDuration = 0
            for _,fn in ipairs(d.holdCallbacks) do pcall(fn) end
            for _r=1,6 do
                for _,fn in ipairs(d.triggerCallbacks) do pcall(fn) end
                if fireproximityprompt then pcall(fireproximityprompt,p) end
            end
            for _,fn in ipairs(d.holdEndCallbacks) do pcall(fn) end
            p.HoldDuration = od
        elseif fireproximityprompt then pcall(fireproximityprompt,p) end
    end
    _G._instantStealOnArrival = function(pd)
        if not pd then return end
        local pr = PromptMemoryCache[pd.uid]
        if not pr or not pr.Parent then pr = findProximityPromptForAnimal(pd) end
        if pr then buildStealCallbacks(pr); executeInstantSteal(pr) end
    end
    ...
    local function scanSinglePlot(plot)
        ...
        if changed then
            ...
            -- Instantly attempt steal the moment a new animal is detected
            if autoStealEnabled and stealPriorityEnabled then
                task.spawn(function()
                    -- Find the highest priority match in the updated cache
                    local targetAnimal = nil
                    for _, pName in ipairs(PRIORITY_LIST) do
                        local searchName = pName:lower()
                        for _, a in ipairs(allAnimalsCache) do
                            if a.name and a.name:lower() == searchName and not isMyBaseAnimal(a) then
                                targetAnimal = a
                                break
                            end
                        end
                    ...
                    -- Fall back to highest gen if no priority match
                    if not targetAnimal then
                        for _, a in ipairs(allAnimalsCache) do
                            if not isMyBaseAnimal(a) then targetAnimal = a; break end
                        end
                    end
                    if not targetAnimal then return end
                    -- Get or find the prompt and fire immediately
                    local pr = PromptMemoryCache[targetAnimal.uid]
                    if not pr or not pr.Parent then
                        pr = findProximityPromptForAnimal(targetAnimal)
                    end
                    if pr then
                        -- Update selection to this target
                        for i, p in ipairs(get_all_pets()) do
                            if p.uid == targetAnimal.uid then
                                selectedTargetIndex = i
                                selectedTargetUID = targetAnimal.uid
                                break
                            end
                        end
                        -- TP to the target immediately then start stealing
                        task.spawn(runAutoSnipe)
                        -- Fire steal immediately without waiting for tween
                        buildStealCallbacks(pr)
                        local data = InternalStealCache[pr]
                        local oDur = pr.HoldDuration
                        pr.HoldDuration = 0
                        if data then
                            for _, fn in ipairs(data.holdCallbacks) do pcall(fn) end
                            for _, fn in ipairs(data.triggerCallbacks) do pcall(fn) end
                        ...
                        if fireproximityprompt then
                            for _rep = 1, 6 do pcall(fireproximityprompt, pr) end
                        end
                        pr.HoldDuration = oDur
                    end
                end)
            end
    ...
    local function ShowPriorityAlert(brainrotName, genText, mutation, ownerUsername)
        ...
        local existing = PlayerGui:FindFirstChild("FlashHubPriorityAlert")
        if existing then existing:Destroy() end
        
        local alertGui = Instance.new("ScreenGui")
        alertGui.Name = "FlashHubPriorityAlert"
    ...
    -- On every respawn, reset and immediately re-enable instant steal
    LocalPlayer.CharacterAdded:Connect(function()
        instantStealDidInit = false
        instantStealReady = false
        -- Set ready immediately so stealing starts the moment character loads
        task.defer(function()
...
task.spawn(function()
    ...
    adminGui.Name = "FlashHubAdminPanel"
    ...
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 12)
    local stroke = Instance.new("UIStroke", frame)
    stroke.Color = Theme.Accent2; pcall(function() stroke:SetAttribute("_acc","A2col") end); stroke.Thickness = 1.5; stroke.Transparency = 0.4
    ...
    refreshStroke.Color = Theme.Accent2; pcall(function() refreshStroke:SetAttribute("_acc","A2col") end)
    ...
    proxContStroke.Color = Theme.Accent2; pcall(function() proxContStroke:SetAttribute("_acc","A2col") end)
    ...
    spamBaseBtn.MouseButton1Click:Connect(function()
        ...
        spamBaseBtn.BackgroundColor3 = Theme.Accent1; pcall(function() spamBaseBtn:SetAttribute("_acc","A1bg") end)
    ...
    proxFill.BackgroundColor3 = Theme.Accent1; pcall(function() proxFill:SetAttribute("_acc","A1bg") end); proxFill.Size = UDim2.new(0,0,1,0)
    ...
    proxKnobStroke.Color = Theme.Accent1; pcall(function() proxKnobStroke:SetAttribute("_acc","A1col") end)
    ...
    local function updateProxViz()
        if ProximityAPActive then 
            if not proxViz then
                ...
                proxViz.Color = Theme.Accent1; pcall(function() proxViz:SetAttribute("_acc","A1col") end); proxViz.Transparency = 0.6
...
isOnCooldown = function(cmd)
    ...
    local function createPlayerRow(plr)
        ...
        rowStroke.Color = Theme.Accent2; pcall(function() rowStroke:SetAttribute("_acc","A2col") end)
        ...
        row.MouseEnter:Connect(function()
            ...
            rowStroke.Color = Theme.Accent1; pcall(function() rowStroke:SetAttribute("_acc","A1col") end)
        end)
        ...
        headshotStroke.Color = Theme.Accent1; pcall(function() headshotStroke:SetAttribute("_acc","A1col") end)
        ...
        btnCont.Size = UDim2.new(0, 178, 1, 0)
        btnCont.Position = UDim2.new(1, -183, 0, 0)
        btnCont.BackgroundTransparency = 1
        btnCont.ZIndex = 10

        -- TP button — teleports to this player's base
        local tpBtn = Instance.new("TextButton", btnCont)
        tpBtn.Size = UDim2.new(0, 34, 0, 30)
        tpBtn.Position = UDim2.new(0, 0, 0.5, -15)
        ...
        tpBtn.ZIndex = 11
        tpBtn.Active = true
        tpBtn.BackgroundColor3 = Color3.fromRGB(30, 80, 160)
        tpBtn.BackgroundTransparency = 0
        Instance.new("UICorner", tpBtn).CornerRadius = UDim.new(0, 8)
        local tpStroke = Instance.new("UIStroke", tpBtn)
        tpStroke.Color = Color3.fromRGB(60, 130, 255)
        ...
        tpBtn.MouseButton1Click:Connect(function()
            task.spawn(function()
                -- Always reset so every press works
                State.isTpMoving = false
                isTpMoving = false

                -- Coordinates (same as global constants defined later)
                local _BASES_LOW = {
                ...
                local function _closestIdx(pos)
                    local best,dist = 1, math.huge
                    for i,v in pairs(_BASES_LOW) do
                        local d = (Vector2.new(pos.X,pos.Z)-Vector2.new(v.X,v.Z)).Magnitude
                ...
                local Plots = Workspace:FindFirstChild("Plots")
                if not Plots then ShowNotification("TP TO BASE","No plots found"); return end
                ...
                if ok and Sync then
                    for _, plot in ipairs(Plots:GetChildren()) do
                        local ok2, ch = pcall(function() return Sync:Get(plot.Name) end)
                        if ok2 and ch then
                            local owner = ch:Get("Owner")
                            local ownerId = (typeof(owner)=="Instance" and owner:IsA("Player")) and owner.UserId
                                or (type(owner)=="table" and owner.UserId)
                            if ownerId == plr.UserId then targetPlot = plot; break end
                        end
                    end
                end
                if not targetPlot then
                    for _, plot in ipairs(Plots:GetChildren()) do
                        ...
                        if lbl and (lbl.Text:lower():find(plr.Name:lower(),1,true)
                                 or lbl.Text:lower():find(plr.DisplayName:lower(),1,true)) then
                            targetPlot = plot; break
                        end
                    end
                end
                if not targetPlot then ShowNotification("TP TO BASE","Could not find "..plr.DisplayName.."'s base"); return end

                -- Resolve a world position to TP to:
                -- prefer their highest-gen brainrot, fall back to plot Base Spawn
                local targetPos = nil
                local cache = SharedState.AllAnimalsCache
                if cache then
                    local best = nil
                    for _, a in ipairs(cache) do
                        if a and a.owner == plr.Name then
                            if not best or a.genValue > best.genValue then best = a end
                        end
                    end
                    if best then
                        -- Try to get the actual world part for this animal
                        pcall(function()
                            ...
                            local spawn   = base    and base:FindFirstChild("Spawn")
                            if spawn then targetPos = spawn.Position
                            elseif base then
                ...
                -- Fallback: plot Base Spawn
                if not targetPos then
                    pcall(function()
                        local base  = targetPlot:FindFirstChild("Base")
                        local spawn = base and base:FindFirstChild("Spawn")
                        if spawn then targetPos = spawn.Position
                        elseif base then
                            local bp = base:FindFirstChildWhichIsA("BasePart")
                ...
                -- Absolute fallback: plot pivot
                if not targetPos then
                    pcall(function() targetPos = targetPlot:GetPivot().Position end)
                end
                if not targetPos then ShowNotification("TP TO BASE","Could not resolve position"); return end
                ...
                ShowNotification("TP TO BASE","Teleporting to "..plr.DisplayName.."'s base")
                ...
                local isSecondFloor = targetPos.Y > 10
                local plotIdx       = _closestIdx(targetPos)
                local basePos       = isSecondFloor and _BASES_HIGH[plotIdx] or _BASES_LOW[plotIdx]
                local minH          = 50
                local targetH       = math.max(basePos.Y, minH)

                -- Phase 1: blast upward
                local t0 = tick()
                while hrp.Position.Y < targetH and (tick()-t0) < 3 do
                    hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 200, hrp.AssemblyLinearVelocity.Z)
                    RunService.Heartbeat:Wait()
                end

                -- Phase 2: CFrame to base
                for _ = 1, 6 do
                    hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
                    if (hrp.Position - basePos).Magnitude > 3 then
                        hrp.CFrame = CFrame.new(basePos)
                        task.wait(0.05)
                    end
                end

                -- Phase 3: CFrame to nearest clone spot (ground floor only)
                if not isSecondFloor then
                    local bestSpot, minDst = _CLONE_POS[1], math.huge
                    for _, v in ipairs(_CLONE_POS) do
                        local d = (targetPos - v).Magnitude
                ...
                -- Phase 4: face the base and stop here (outside)
                local bestFace, minFD = _FACE[1], math.huge
                for _, v in ipairs(_FACE) do
                    local d = (hrp.Position - v).Magnitude
                    if d < minFD then minFD=d; bestFace=v end
                end
                task.wait(0.08)
                hrp.CFrame = CFrame.lookAt(hrp.Position, Vector3.new(bestFace.X, hrp.Position.Y, bestFace.Z))
                hrp.AssemblyLinearVelocity = Vector3.zero
                ShowNotification("TP TO BASE", "Arrived at "..plr.DisplayName.."'s base")
            end)
        end)
            ...
            task.spawn(function()
                while b and b.Parent do
                    ...
                    elseif not cd and not balloon then
                        b.BackgroundColor3 = Color3.fromRGB(35, 37, 43)
                        b.BackgroundTransparency = 0
                        bStroke.Color = Theme.Accent1; pcall(function() bStroke:SetAttribute("_acc","A1col") end)
                        bStroke.Transparency = 0.4
                        if b.Text ~= def.icon then
                            b.Text = def.icon
                            b.TextSize = 18
        ...
        rowHighlight.BackgroundColor3 = Theme.Accent1; pcall(function() rowHighlight:SetAttribute("_acc","A1bg") end)
...
UserInputService.InputBegan:Connect(function(input, gp)
	...
	if input.KeyCode == (Config.FloatKey and Enum.KeyCode[Config.FloatKey] or Enum.KeyCode.G) then
		if _G.FloatEnabled then
...
local function runAutoSnipe()
    ...
    -- Anti TP-Die: keep health full and block death events during entire TP
    local _tpAntiDieConn = nil
    local _tpDiedConn = nil
    local function startTpAntiDie(character)
        if not character then return end
        local h = character:FindFirstChildOfClass("Humanoid")
        if not h then return end
        if _tpAntiDieConn then pcall(function() _tpAntiDieConn:Disconnect() end) end
        _tpAntiDieConn = h:GetPropertyChangedSignal("Health"):Connect(function()
            if State.isTpMoving and h.Health < h.MaxHealth then
                pcall(function() h.Health = h.MaxHealth end)
            end
        end)
        if _tpDiedConn then pcall(function() _tpDiedConn:Disconnect() end) end
        _tpDiedConn = h.Died:Connect(function()
    ...
    local function stopTpAntiDie()
        if _tpAntiDieConn then pcall(function() _tpAntiDieConn:Disconnect() end); _tpAntiDieConn = nil end
        if _tpDiedConn    then pcall(function() _tpDiedConn:Disconnect()    end); _tpDiedConn    = nil end
    end
    startTpAntiDie(LocalPlayer.Character)

    local targetPart = findAdorneeGlobal(targetPetData)
    if not targetPart then
        stopTpAntiDie(); State.isTpMoving = false; isTpMoving = State.isTpMoving; return
    end
    ...
    -- Phase 2: CFrame to base position
    for i = 1, 6 do
        hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
    ...
    -- Phase 3: CFrame to clone spot (ground floor only)
    if not isSecondFloor then
    ...
    -- Phase 5: clone if 2nd floor or locked plot
    ...
    if verticalDiff > 2 then
        ...
        pinConn = RunService.Heartbeat:Connect(function()
            if LocalPlayer:GetAttribute("Stealing") then
                pinConn:Disconnect(); _G._activePinConn = nil; return
            end
            plat.Position = Vector3.new(targetPart.Position.X, targetPart.Position.Y - (Config.PlatformOffset or 12.5), targetPart.Position.Z)
        end)
        ...
        task.spawn(function()
            ...
            _G._activePinConn = nil; _G._activePlat = nil
            if plat and plat.Parent then plat:Destroy() end
        end)
    else
        -- Pet is at same level — direct snap
        for i = 1, 10 do
            if LocalPlayer:GetAttribute("Stealing") then break end
            hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
    ...
    if _G._instantStealOnArrival and targetPetData then
        task.spawn(function()
            task.wait(0.05)
            for _i = 1, 8 do
                if LocalPlayer:GetAttribute("Stealing") then break end
                _G._instantStealOnArrival(targetPetData)
                task.wait(0.04)
            end
        end)
    end
...
local function executeReset()
    ...
    task.spawn(function()
        ...
        -- 1. Disable AntiDie so Health=0 actually kills
        local antiDieWasDisabled = _G.AntiDieDisabled
        ...
        -- 2. Drop brainrot/tools
        pcall(function() hum:UnequipTools() end)

        -- 3. Clear all base locks
        ...
        -- 5. Blast upward 2 frames then instantly CFrame above barrier
        --    2 frames is enough to break free, no need to wait 0.5s
        pcall(function() hrp.AssemblyLinearVelocity = Vector3.new(0, 10000000, 0) end)
        RunService.Heartbeat:Wait()
        pcall(function() hrp.AssemblyLinearVelocity = Vector3.new(0, 10000000, 0) end)
        RunService.Heartbeat:Wait()

        -- 6. Instant CFrame above barrier — no loop needed
        pcall(function()
        ...
        -- 7. Kill immediately — AntiDie is off so Health=0 fires instantly
        --    Also force RespawnTime to 0 right before kill so server respawns instantly
        pcall(function() game:GetService("Players").RespawnTime = 0 end)
        ...
        if char then
            ...
            -- Only wait for HRP fallback if still alive after 1 frame
            RunService.Heartbeat:Wait()
            char = LocalPlayer.Character
            ...
            if h and h.Health > 0 then
                pcall(function()
                    ...
                    if hrp2 then hrp2:Destroy() end
                end)
            end
        end

        -- 8. Force immediate respawn via LoadCharacter instead of waiting for server
        pcall(function() LocalPlayer:LoadCharacter() end)

        -- 9. Wait for the new character using CharacterAdded event
        local newChar
        local charConn
        ...
        -- Check if already respawned (LoadCharacter may have already fired it)
        if LocalPlayer.Character and LocalPlayer.Character ~= char then
            newChar = LocalPlayer.Character
            charReady = true
        end
        local t = tick()
        repeat RunService.Heartbeat:Wait() until charReady or (tick() - t > 3)
        pcall(function() charConn:Disconnect() end)

        newChar = newChar or LocalPlayer.Character
        if newChar then
            -- Wait just 1 frame for the character to initialize
            RunService.Heartbeat:Wait()
            local newHum = newChar:FindFirstChildOfClass("Humanoid")
            if newHum then
                pcall(function() newHum.PlatformStand = false end)
                pcall(function() newHum.Sit           = false end)
                pcall(function() newHum.WalkSpeed     = 16   end)
                pcall(function() newHum.JumpPower     = 50   end)
        ...
        -- 9. Re-enable AntiDie for the new character
...
-- Instant balloon reset: fires the exact frame the balloon GUI appears
do
    local balloonPhrase = 'ran "balloon" on you'
    local _balloonResetFired = false
    RunService.Heartbeat:Connect(function()
        if not Config.AutoResetOnBalloon then
            _balloonResetFired = false
            return
        end
        for _, gui in ipairs(PlayerGui:GetDescendants()) do
            if (gui:IsA("TextLabel") or gui:IsA("TextButton")) and gui.Text and gui.Text:find(balloonPhrase, 1, true) then
                if not _balloonResetFired then
                    _balloonResetFired = true
                    executeReset()
                end
                return
            end
        end
        -- Reset debounce once the balloon GUI disappears
        _balloonResetFired = false
    end)
end

-- Insta kick: fires the exact frame "You stole" GUI is created
PlayerGui.DescendantAdded:Connect(function(obj)
    if not Config.AutoKickOnSteal then return end
    if (obj:IsA("TextLabel") or obj:IsA("TextButton")) and obj.Text and obj.Text:find("You stole") then
        kickPlayer()
    end
end)

-- Fallback: also watch for text property changes (some games set text after creation)
PlayerGui.DescendantAdded:Connect(function(obj)
    if not Config.AutoKickOnSteal then return end
    if obj:IsA("TextLabel") or obj:IsA("TextButton") then
        obj:GetPropertyChangedSignal("Text"):Connect(function()
            if Config.AutoKickOnSteal and obj.Text and obj.Text:find("You stole") then
                kickPlayer()
            end
        end)
    end
...
if IS_MOBILE then
    ...
    mobileGui.Name = "FlashHubMobileControls"
    ...
    cStroke.Color = Theme.Accent1; pcall(function() cStroke:SetAttribute("_acc","A1col") end)
    ...
    createMobBtn("UI", Color3.fromRGB(255, 255, 255), 5, function()
        ...
        local adUI = PlayerGui:FindFirstChild("FlashHubAdminPanel")
    ...
    openBtn.BackgroundColor3 = Theme.Accent1; pcall(function() openBtn:SetAttribute("_acc","A1bg") end)
    ...
    openStroke.Color = Theme.Accent2; pcall(function() openStroke:SetAttribute("_acc","A2col") end)
...
sStroke.Color = Theme.Accent2; pcall(function() sStroke:SetAttribute("_acc","A2col") end); sStroke.Thickness = 1.5; sStroke.Transparency = 0.4
...
local function CreateSectionHeader(text)
    ...
    accent.BackgroundColor3 = Theme.Accent1; pcall(function() accent:SetAttribute("_acc","A1bg") end)
    ...
    lbl.Text = text
    lbl.TextColor3 = Theme.Accent1; pcall(function() lbl:SetAttribute("_acc","A1tx") end)
    lbl.TextSize = 11
    ...
    line.BackgroundColor3 = Theme.Accent1; pcall(function() line:SetAttribute("_acc","A1bg") end)
...
local rTpOnReset = CreateRow("Auto TP After Reset")
CreateToggleSwitch(rTpOnReset, Config.TpSettings.TpOnReset, function(ns, set)
    set(ns); Config.TpSettings.TpOnReset = ns; SaveConfig()
    ShowNotification("AUTO TP AFTER RESET", ns and "ENABLED" or "DISABLED")
end)
...
bBind.MouseButton1Click:Connect(function()
    bBind.Text="..."; bBind.TextColor3 = Theme.Accent1; pcall(function() bBind:SetAttribute("_acc","A1tx") end)
...
bBindClone.MouseButton1Click:Connect(function()
    bBindClone.Text="..."; bBindClone.TextColor3 = Theme.Accent1; pcall(function() bBindClone:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
bCarpet.MouseButton1Click:Connect(function()
    bCarpet.Text="..."; bCarpet.TextColor3 = Theme.Accent1; pcall(function() bCarpet:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
bRagdollSelf.MouseButton1Click:Connect(function()
    bRagdollSelf.Text="..."; bRagdollSelf.TextColor3 = Theme.Accent1; pcall(function() bRagdollSelf:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
bStealSpeedKey.MouseButton1Click:Connect(function()
    bStealSpeedKey.Text="..."; bStealSpeedKey.TextColor3 = Theme.Accent1; pcall(function() bStealSpeedKey:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
local floatToggleSwitch = CreateToggleSwitch(rFloat, _G.FloatEnabled or false, function(ns, set)
    set(ns)
    if ns then
        if _G.enableFloat then pcall(_G.enableFloat) end
    else
    ...
    if _G.updateMovementPanelFloatVisual then pcall(_G.updateMovementPanelFloatVisual, ns) end
    ShowNotification("FLOAT", ns and "ENABLED" or "DISABLED")
end)
_G.updateMovementPanelFloatVisual = function(state)
    if floatToggleSwitch and floatToggleSwitch.Set then
        floatToggleSwitch.Set(state)
    end
end
...
bFloatKey.BackgroundColor3=Theme.SurfaceHighlight; bFloatKey.Text=Config.FloatKey or "G"
bFloatKey.Font=Enum.Font.GothamBold; bFloatKey.TextColor3=Theme.TextPrimary; bFloatKey.TextSize=12
Instance.new("UICorner",bFloatKey).CornerRadius=UDim.new(0,4)
bFloatKey.MouseButton1Click:Connect(function()
    bFloatKey.Text="..."; bFloatKey.TextColor3 = Theme.Accent1; pcall(function() bFloatKey:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
do
    local rAutoBuy = CreateRow("Auto Buy")
    CreateToggleSwitch(rAutoBuy, Config.AutoBuyEnabled, function(ns, set)
        set(ns); Config.AutoBuyEnabled = ns; SaveConfig()
        if _G._autoBuyToggle then
            if ns and not _G._autoBuyActive then task.spawn(_G._autoBuyToggle)
            elseif not ns and _G._autoBuyActive then _G._autoBuyToggle() end
        end
        ShowNotification("AUTO BUY", ns and "ENABLED" or "DISABLED")
    end)
    local rAutoBuyBind = CreateRow("Auto Buy Keybind")
    do
        local bAutoBuy = Instance.new("TextButton", rAutoBuyBind)
        bAutoBuy.Size = UDim2.new(0,60,0,24); bAutoBuy.Position = UDim2.new(1,-70,0.5,-12)
        bAutoBuy.BackgroundColor3 = Theme.SurfaceHighlight; bAutoBuy.Text = Config.AutoBuyKey
        ...
        bAutoBuy.MouseButton1Click:Connect(function()
            bAutoBuy.Text = "..."; bAutoBuy.TextColor3 = Theme.Accent1; pcall(function() bAutoBuy:SetAttribute("_acc","A1tx") end)
            local con; con = UserInputService.InputBegan:Connect(function(inp, proc)
                if proc then return end
                if inp.UserInputType == Enum.UserInputType.Keyboard then
                    Config.AutoBuyKey = inp.KeyCode.Name
                    bAutoBuy.Text = inp.KeyCode.Name
                    bAutoBuy.TextColor3 = Theme.TextPrimary
...
do
    local rAutoBuyKick = CreateRow("Auto Buy Kick")
    CreateToggleSwitch(rAutoBuyKick, Config.AutoBuyKick, function(ns, set)
        set(ns); Config.AutoBuyKick = ns; SaveConfig()
        ShowNotification("AUTO BUY KICK", ns and "ENABLED" or "DISABLED")
    end)
    local rAutoBuyKickBind = CreateRow("Auto Buy Kick Keybind")
    local bAutoBuyKick = Instance.new("TextButton", rAutoBuyKickBind)
    bAutoBuyKick.Size = UDim2.new(0,60,0,24); bAutoBuyKick.Position = UDim2.new(1,-70,0.5,-12)
    bAutoBuyKick.BackgroundColor3 = Theme.SurfaceHighlight
    bAutoBuyKick.Text = Config.AutoBuyKickKey ~= "" and Config.AutoBuyKickKey or "NONE"
    bAutoBuyKick.Font = Enum.Font.GothamBold; bAutoBuyKick.TextColor3 = Theme.TextPrimary; bAutoBuyKick.TextSize = 12
    Instance.new("UICorner", bAutoBuyKick).CornerRadius = UDim.new(0,4)
    bAutoBuyKick.MouseButton1Click:Connect(function()
        bAutoBuyKick.Text = "..."; bAutoBuyKick.TextColor3 = Theme.Accent1
        local con; con = UserInputService.InputBegan:Connect(function(inp, proc)
            if proc then return end
            if inp.UserInputType == Enum.UserInputType.Keyboard then
                Config.AutoBuyKickKey = inp.KeyCode.Name
                bAutoBuyKick.Text = inp.KeyCode.Name
                bAutoBuyKick.TextColor3 = Theme.TextPrimary
                SaveConfig(); con:Disconnect()
                ShowNotification("AUTO BUY KICK KEYBIND", inp.KeyCode.Name)
            end
        end)
    end)
end
...
bResetKey.MouseButton1Click:Connect(function()
    bResetKey.Text="..."; bResetKey.TextColor3 = Theme.Accent1; pcall(function() bResetKey:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
bKickKey.MouseButton1Click:Connect(function()
    bKickKey.Text="..."; bKickKey.TextColor3 = Theme.Accent1; pcall(function() bKickKey:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
bClickToAPKeybind.MouseButton1Click:Connect(function()
    bClickToAPKeybind.Text="..."; bClickToAPKeybind.TextColor3 = Theme.Accent1; pcall(function() bClickToAPKeybind:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
bProximityAPKeybind.MouseButton1Click:Connect(function()
    bProximityAPKeybind.Text="..."; bProximityAPKeybind.TextColor3 = Theme.Accent1; pcall(function() bProximityAPKeybind:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
bJoinerKey.MouseButton1Click:Connect(function()
    bJoinerKey.Text="..."; bJoinerKey.TextColor3 = Theme.Accent1; pcall(function() bJoinerKey:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
local rRejoinBtn = CreateRow("Rejoin Server")
local bRejoin = Instance.new("TextButton", rRejoinBtn)
bRejoin.Size=UDim2.new(0,70,0,24); bRejoin.Position=UDim2.new(1,-80,0.5,-12)
bRejoin.BackgroundColor3 = Theme.Accent1; pcall(function() bRejoin:SetAttribute("_acc","A1bg") end); bRejoin.Text="REJOIN"
bRejoin.Font=Enum.Font.GothamBold; bRejoin.TextColor3=Color3.new(0,0,0); bRejoin.TextSize=12
Instance.new("UICorner",bRejoin).CornerRadius=UDim.new(0,4)
...
bRejoinKey.BackgroundColor3=Theme.SurfaceHighlight; bRejoinKey.Text=Config.RejoinKey or "F4"
bRejoinKey.Font=Enum.Font.GothamBold; bRejoinKey.TextColor3=Theme.TextPrimary; bRejoinKey.TextSize=12
Instance.new("UICorner",bRejoinKey).CornerRadius=UDim.new(0,4)
bRejoinKey.MouseButton1Click:Connect(function()
    bRejoinKey.Text="..."; bRejoinKey.TextColor3 = Theme.Accent1; pcall(function() bRejoinKey:SetAttribute("_acc","A1tx") end)
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
...
fovFill.BackgroundColor3 = Theme.Accent1; pcall(function() fovFill:SetAttribute("_acc","A1bg") end)
...
fovKnobStroke.Color = Theme.Accent1; pcall(function() fovKnobStroke:SetAttribute("_acc","A1col") end)
...
if not IS_MOBILE then
    ...
    bMenu.MouseButton1Click:Connect(function()
        bMenu.Text="..."; bMenu.TextColor3 = Theme.Accent1; pcall(function() bMenu:SetAttribute("_acc","A1tx") end)
        local con; con=UserInputService.InputBegan:Connect(function(inp)
            if inp.UserInputType==Enum.UserInputType.Keyboard then
...
if IS_MOBILE then
    ...
    scaleUI.fill.BackgroundColor3 = Theme.Accent1; pcall(function() scaleUI.fill:SetAttribute("_acc","A1bg") end)
    ...
    scaleUI.stroke.Color = Theme.Accent1; pcall(function() scaleUI.stroke:SetAttribute("_acc","A1col") end)
    ...
    scaleUI.readout.TextColor3 = Theme.Accent1; pcall(function() scaleUI.readout:SetAttribute("_acc","A1tx") end)
...
bReset.MouseButton1Click:Connect(function()
    ...
    if PlayerGui:FindFirstChild("FlashHubAdminPanel") and PlayerGui.XiAdminPanel:FindFirstChild("Frame") then
...
CreateSectionHeader("THEME COLOR")
pcall(function()
    -- All accent colour pairs: {Accent1, Accent2}
    local _TM = {
        Red    = {Color3.fromRGB(255,50,50),   Color3.fromRGB(200,30,30)},
        Blue   = {Color3.fromRGB(50,130,255),  Color3.fromRGB(30,90,200)},
        Pink   = {Color3.fromRGB(255,80,180),  Color3.fromRGB(200,40,140)},
    ...
    local _TC = {
        {n="Red",c=_TM.Red[1]},{n="Blue",c=_TM.Blue[1]},{n="Pink",c=_TM.Pink[1]},
        {n="Green",c=_TM.Green[1]},{n="Orange",c=_TM.Orange[1]},{n="Gold",c=_TM.Gold[1]},
        {n="White",c=_TM.White[1]},{n="Black",c=_TM.Black[1]},
    }


    -- Swatch row
    local _row = CreateRow("Color", 44)
    local _sc = Instance.new("Frame", _row)
    _sc.Size=UDim2.new(1,-10,0,26); _sc.Position=UDim2.new(0,5,1,-30)
    _sc.BackgroundTransparency=1
    local _sl = Instance.new("UIListLayout", _sc)
    _sl.FillDirection=Enum.FillDirection.Horizontal
    _sl.Padding=UDim.new(0,4); _sl.SortOrder=Enum.SortOrder.LayoutOrder
    local _strokes = {}
    for _idx, _tc in ipairs(_TC) do
        local _sw = Instance.new("TextButton", _sc)
        _sw.Size=UDim2.new(0,26,0,26); _sw.BackgroundColor3=_tc.c
        _sw.Text=""; _sw.BorderSizePixel=0; _sw.AutoButtonColor=false
        _sw.LayoutOrder=_idx
        Instance.new("UICorner", _sw).CornerRadius=UDim.new(0,5)
        local _s = Instance.new("UIStroke", _sw)
        _s.Thickness=2; _s.Color=Color3.new(1,1,1)
        _s.Transparency = (Config.ThemeColor==_tc.n) and 0.1 or 1
        _strokes[_tc.n] = _s
        local _cap = _tc
        _sw.MouseEnter:Connect(function() _s.Transparency=0.2 end)
        _sw.MouseLeave:Connect(function()
            _s.Transparency=(Config.ThemeColor==_cap.n) and 0.1 or 1
        end)
        _sw.MouseButton1Click:Connect(function()
            local _pair = _TM[_cap.n]
            if not _pair then return end
            Theme.Accent1 = _pair[1]; Theme.Accent2 = _pair[2]
            Config.ThemeColor = _cap.n; SaveConfig()
            if _G._repaintAccents then _G._repaintAccents() end
            for _, _st in pairs(_strokes) do _st.Transparency=1 end
            _s.Transparency=0.1
            ShowNotification("THEME", _cap.n:upper().." applied")
        end)
    end
end)
...
if not IS_MOBILE then
    UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if Config.RejoinKey and Config.RejoinKey ~= "" and input.KeyCode == Enum.KeyCode[Config.RejoinKey] then
            ShowNotification("REJOIN", "Rejoining server...")
            task.delay(0.5, function()
                TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlayer)
            end)
        end
        if Config.AutoBuyKey and Config.AutoBuyKey ~= "" and input.KeyCode == Enum.KeyCode[Config.AutoBuyKey] then
            if _G._autoBuyToggle then
                _G._autoBuyToggle()
            else
                ShowNotification("AUTO BUY", "Still loading, try again shortly")
            end
        end
        if Config.AutoBuyKickKey and Config.AutoBuyKickKey ~= "" and input.KeyCode == Enum.KeyCode[Config.AutoBuyKickKey] then
            Config.AutoBuyKick = not Config.AutoBuyKick; SaveConfig()
            ShowNotification("AUTO BUY KICK", Config.AutoBuyKick and "ENABLED" or "DISABLED")
        end
    end)
end
...
-- Auto TP after reset — same logic as TpOnLoad, fires on every respawn
pcall(function()
    local _tpOnResetFirstSpawn = true
    LocalPlayer.CharacterAdded:Connect(function()
        if _tpOnResetFirstSpawn then
            _tpOnResetFirstSpawn = false
            return
        end
        if not Config.TpSettings.TpOnReset then return end
        task.spawn(function()
            task.wait(0.3)
            local t = 0
            while not SharedState.SelectedPetData and t < 150 do
            ...
            if minGen > 0 then
                ...
                local highestGen = (cache[1] and cache[1].genValue) or 0
                if highestGen < minGen then
                    ShowNotification("AUTO TP RESET", "No brainrot above min gen, skipping.")
                    return
                end
            end
            ShowNotification("AUTO TP", "Teleporting after reset...")
            runAutoSnipe()
        end)
    end)
end)
...
LocalPlayer:GetAttributeChangedSignal("Stealing"):Connect(function()
    ...
    if isStealing then
        ...
        if Config.AutoUnlockOnSteal then
            -- Detect which floor the target pet is on and open that door
            task.spawn(function()
                local petData = SharedState.SelectedPetData and SharedState.SelectedPetData.animalData
                local petPart = petData and findAdorneeGlobal and findAdorneeGlobal(petData)
                local petY = petPart and petPart.Position.Y or 0
                -- Floor index: below 10 = floor 1, 10-23 = floor 2, above 23 = floor 3
                local floorIdx = 1
                if petY > 23 then
                    floorIdx = 3
                elseif petY > 10 then
                ...
                -- Find target plot and fire its Nth door prompt
                local plots = Workspace:FindFirstChild("Plots")
                if not plots then return end
                local targetPlotName = petData and petData.plot
                local targetPlot = nil
                if targetPlotName then
                    for _, plot in ipairs(plots:GetChildren()) do
                        if plot.Name == targetPlotName then targetPlot = plot; break end
                    end
                end
                -- Fallback: nearest plot to pet
                if not targetPlot and petPart then
                    local minD = math.huge
                    for _, plot in ipairs(plots:GetChildren()) do
                        local pp = pcall(function() return plot:GetPivot().Position end) and plot:GetPivot().Position
                        if pp then
                            local d = (petPart.Position - pp).Magnitude
                            if d < minD then minD = d; targetPlot = plot end
                        end
                    end
                end
                if not targetPlot then return end
                -- Collect prompts sorted by Y
                local prompts = {}
                for _, desc in ipairs(targetPlot:GetDescendants()) do
                    if desc:IsA("ProximityPrompt") then
                        local part = desc.Parent
                        if part and part:IsA("BasePart") then
                            table.insert(prompts, {prompt = desc, y = part.Position.Y})
                        end
                    end
                end
                table.sort(prompts, function(a, b) return a.y < b.y end)
                local target = prompts[floorIdx]
...
task.spawn(function()
    ...
    ssStroke.Color = Theme.Accent2; pcall(function() ssStroke:SetAttribute("_acc","A2col") end); ssStroke.Thickness=1.5; ssStroke.Transparency=0.4
    ...
    speedReadout.TextColor3 = Theme.Accent1; pcall(function() speedReadout:SetAttribute("_acc","A1tx") end); speedReadout.TextXAlignment=Enum.TextXAlignment.Right
    ...
    sliderFill.BackgroundColor3 = Theme.Accent1; pcall(function() sliderFill:SetAttribute("_acc","A1bg") end); sliderFill.BorderSizePixel=0; sliderFill.Size=UDim2.new(0,0,1,0)
    ...
    local knobStroke=Instance.new("UIStroke",knob); knobStroke.Color = Theme.Accent1; pcall(function() knobStroke:SetAttribute("_acc","A1col") end); knobStroke.Thickness=2
...
task.spawn(function()
    ...
    iStroke.Color = Theme.Accent2; pcall(function() iStroke:SetAttribute("_acc","A2col") end)
    ...
    keyBtn.TextColor3 = Theme.Accent1; pcall(function() keyBtn:SetAttribute("_acc","A1tx") end)
    keyBtn.TextSize = 11
    ...
    local function CreateFancySlider(parent, name, min, max, default, callback)
        ...
        if IS_MOBILE then
            ...
            textBoxStroke.Color = Theme.Accent1; pcall(function() textBoxStroke:SetAttribute("_acc","A1col") end)
...
task.spawn(function()
    ...
    -- Floor buttons: open the Nth door on YOUR OWN plot sorted by height (floor 1 = lowest door)
    local function openMyPlotFloor(floorIndex)
        local plots = Workspace:FindFirstChild("Plots")
        if not plots then return end
        -- Find my plot
        local myPlot = nil
        for _, plot in ipairs(plots:GetChildren()) do
            local sign = plot:FindFirstChild("PlotSign")
            if sign then
                local surf = sign:FindFirstChildWhichIsA("SurfaceGui", true)
                local lbl = surf and surf:FindFirstChildWhichIsA("TextLabel", true)
                if lbl then
        ...
        if not myPlot then
            ShowNotification("UNLOCK", "Could not find your base")
            return
        end
        local unlockFolder = myPlot:FindFirstChild("Unlock")
        if not unlockFolder then ShowNotification("UNLOCK","No Unlock folder"); return end
        local items = {}
        for _, item in pairs(unlockFolder:GetChildren()) do
            local pos
            if item:IsA("Model") then pcall(function() pos=item:GetPivot().Position end)
        ...
        local door = items[floorIndex]
        if not door then ShowNotification("UNLOCK","Floor "..floorIndex.." not found"); return end
        local prompt
        for _,d in ipairs(door.obj:GetDescendants()) do
            if d:IsA("ProximityPrompt") then prompt=d; break end
        end
        if not prompt then ShowNotification("UNLOCK","No prompt"); return end
        if _G._firePrompt then _G._firePrompt(prompt)
        elseif fireproximityprompt then fireproximityprompt(prompt) end
        ShowNotification("UNLOCK","Floor "..floorIndex.." unlocked")
    end
...
task.spawn(function()
    ...
    local function makePlayerBillboard(player)
        ...
        nameLbl.TextColor3 = Theme.Accent1; pcall(function() nameLbl:SetAttribute("_acc","A1tx") end)
...
task.spawn(function()
    ...
    mainStroke.Color = Theme.Accent2; pcall(function() mainStroke:SetAttribute("_acc","A2col") end)
    ...
    local function refreshPriorityList()
        ...
        for i, petName in ipairs(PRIORITY_LIST) do
            ...
            upBtn.BackgroundColor3 = Theme.Accent1; pcall(function() upBtn:SetAttribute("_acc","A1bg") end)
            upBtn.Text = "↑"
            ...
            downBtn.BackgroundColor3 = Theme.Accent1; pcall(function() downBtn:SetAttribute("_acc","A1bg") end)
            downBtn.Text = "↓"
    ...
    saveBtn.MouseButton1Click:Connect(function()
        -- Save PRIORITY_LIST into Config and write to disk
        Config.PriorityList = {}
...
-- Auto Buy Kick: kick the instant a brainrot lands in your base
task.spawn(function()
    local ok, Sync = pcall(function()
        return require(ReplicatedStorage:WaitForChild("Packages"):WaitForChild("Synchronizer"))
    end)
    ...
    local function snapshotAnimalList(plot)
        local snap = {}
        pcall(function()
            local ch = Sync:Get(plot.Name)
            local al = ch and ch:Get("AnimalList")
            if al then
                for slot, v in pairs(al) do
                    if type(v) == "table" then snap[tostring(slot)] = v.Index end
                end
            end
        end)
        return snap
    ...
    -- Poll every 0.05s — when AutoBuyKick is on and a new animal slot appears, kick instantly
    local lastSnap = {}
    local watchedPlot = nil
    local lastKickTime = 0

    while true do
        RunService.Heartbeat:Wait()
        if not Config.AutoBuyKick then
            lastSnap = {}; watchedPlot = nil
        else
            -- refresh my plot reference
            if not watchedPlot or not watchedPlot.Parent then
                watchedPlot = getMyPlot()
                if watchedPlot then
                    lastSnap = snapshotAnimalList(watchedPlot)
                end
            end
            if watchedPlot and watchedPlot.Parent then
                local currentSnap = snapshotAnimalList(watchedPlot)
                -- detect any new slot that wasn't in the last snapshot
                local newAnimal = false
                for slot, idx in pairs(currentSnap) do
                    if not lastSnap[slot] then
                        newAnimal = true; break
                    end
                end
                lastSnap = currentSnap
                if newAnimal and (tick() - lastKickTime) > 0.1 then
                    lastKickTime = tick()
                    kickPlayer()
                end
            end
...
local function createDesyncESP()
    if anchorHighlight then 
        if anchorHighlight.highlight then 
            anchorHighlight.highlight:Destroy() 
        end
        if anchorHighlight.billboard then 
            anchorHighlight.billboard:Destroy() 
        end
        if anchorHighlight.part then 
            anchorHighlight.part:Destroy()
        end
        if anchorHighlight.starParts then
            for _, part in pairs(anchorHighlight.starParts) do
                part:Destroy()
            end
        end
        anchorHighlight = nil 
    ...
    for i = 1, 4 do
        local starPoint = Instance.new("Part")
        starPoint.Name = "StarPoint" .. i
        starPoint.Anchored = true
        starPoint.CanCollide = false
        starPoint.Shape = Enum.PartType.Ball
        starPoint.Size = Vector3.new(2, 2, 2)
        starPoint.Position = part.Position
        starPoint.Material = Enum.Material.Neon
        starPoint.Color = Color3.fromRGB(255, 255, 100)
        starPoint.Transparency = 0.2
        starPoint.Parent = ESPFolder
        table.insert(starParts, starPoint)
    end
    ...
    posText.TextColor3 = Color3.fromRGB(255, 255, 150)
    ...
    if anchorHighlight and anchorHighlight.part and serverPosition then
        ...
        if character then
            ...
            if hrp then
                ...
                if anchorHighlight.starParts then
                    local time = tick()
                    for i, part in ipairs(anchorHighlight.starParts) do
                        local angle = (i * 1.5) + time * 1.2 
                        local orbitRadius = 4
                        
                        local x = math.cos(angle) * orbitRadius
                        local z = math.sin(angle) * orbitRadius
                        
                        part.Position = anchorHighlight.part.Position + Vector3.new(x, 0, z)
                        
                        local pulse = math.sin(time * 1 + i * 0.5) * 0.1 + 0.9
                        part.Size = Vector3.new(2, 2, 2) * pulse
                    end
                end
            end
        end
...
local function setupPositionTracking(hrp)
...
local function initializeESP()
    ...
    local highlight = createDesyncESP()
...
-- ═══════════════════════════════════════════════════════════════════
-- AUTO BUY MODULE — Standalone GUI, integrated into Flash Hub
-- ═══════════════════════════════════════════════════════════════════
task.spawn(function()
    ...
    local AB_HEIGHT     = 0
    local AB_FIRE_RATE  = 0.05
    local AB_SCAN_INT   = 1.5
    local AB_SNAP_DIST  = 20
    local AB_SMOOTH_SPD = 18
    local AB_REACH      = 60

    -- ── SCREEN GUI ──────────────────────────────────────────────────
    local abSg = Instance.new("ScreenGui")
    abSg.Name            = "FlashHub_AutoBuy"
    abSg.ResetOnSpawn    = false
    abSg.DisplayOrder    = 999998
    abSg.IgnoreGuiInset  = true
    pcall(function() abSg.Parent = game:GetService("CoreGui") end)
    if not abSg.Parent then pcall(function() if gethui then abSg.Parent = gethui() end end) end
    if not abSg.Parent then abSg.Parent = LocalPlayer:WaitForChild("PlayerGui") end
    ...
    local AC = {
        ...
        accent    = Color3.fromRGB(80, 210, 105),
        accentDim = Color3.fromRGB(45, 120, 60),
        red       = Color3.fromRGB(210, 65, 65),
        gold      = Color3.fromRGB(225, 180, 55),
        text      = Color3.fromRGB(220, 220, 228),
        textDim   = Color3.fromRGB(110, 110, 122),
        rowHover  = Color3.fromRGB(28, 28, 36),
        rowActive = Color3.fromRGB(18, 40, 22),
    }

    -- ── UI HELPERS ───────────────────────────────────────────────────
    local function abCorner(r, p)
        local u = Instance.new("UICorner", p); u.CornerRadius = UDim.new(0, r)
    end
    local function abStroke(t, col, p)
        local s = Instance.new("UIStroke", p); s.Thickness = t; s.Color = col
    end
    local function abPad(h, v, p)
        local u = Instance.new("UIPadding", p)
        u.PaddingLeft = UDim.new(0,h); u.PaddingRight  = UDim.new(0,h)
        u.PaddingTop  = UDim.new(0,v); u.PaddingBottom = UDim.new(0,v)
    end
    local function abDrag(handle, target)
    ...
    abPanel.Name             = "AutoBuyPanel"
    abPanel.Size             = UDim2.new(0, 240, 0, 330)
    abPanel.Position         = UDim2.new(0, 16, 0.5, -165)
    abPanel.BackgroundColor3 = AC.bg
    abPanel.BorderSizePixel  = 0
    abPanel.Active           = true
    abPanel.ClipsDescendants = true
    abPanel.Visible          = false
    abPanel.Parent           = abSg
    abCorner(10, abPanel)
    abStroke(1, AC.border, abPanel)

    -- Header bar
    local abHdr = Instance.new("Frame")
    abHdr.Size             = UDim2.new(1, 0, 0, 40)
    abHdr.BackgroundColor3 = AC.surface
    abHdr.BorderSizePixel  = 0
    abHdr.Parent           = abPanel
    abCorner(10, abHdr)
    abDrag(abHdr, abPanel)

    -- Accent line under header
    local abLine = Instance.new("Frame")
    abLine.Size             = UDim2.new(1, 0, 0, 2)
    abLine.Position         = UDim2.new(0, 0, 0, 40)
    abLine.BackgroundColor3 = AC.accent
    abLine.BackgroundTransparency = 0.6
    abLine.BorderSizePixel  = 0
    abLine.Parent           = abPanel

    local abTitle = Instance.new("TextLabel")
    abTitle.Size               = UDim2.new(1, -50, 1, 0)
    abTitle.Position           = UDim2.new(0, 14, 0, 0)
    abTitle.BackgroundTransparency = 1
    abTitle.Text               = "AUTO BUY"
    abTitle.TextColor3         = AC.text
    abTitle.Font               = Enum.Font.GothamBlack
    abTitle.TextSize           = 13
    abTitle.TextXAlignment     = Enum.TextXAlignment.Left
    abTitle.Parent             = abHdr

    -- Status dot in header
    local abDot = Instance.new("TextLabel")
    abDot.Size               = UDim2.new(0, 36, 1, 0)
    abDot.Position           = UDim2.new(1, -40, 0, 0)
    abDot.BackgroundTransparency = 1
    abDot.Text               = "OFF"
    abDot.TextColor3         = AC.red
    abDot.Font               = Enum.Font.GothamBold
    abDot.TextSize           = 10
    abDot.Parent             = abHdr

    -- Found count
    local abCount = Instance.new("TextLabel")
    abCount.Size               = UDim2.new(1, -24, 0, 20)
    abCount.Position           = UDim2.new(0, 12, 0, 46)
    abCount.BackgroundTransparency = 1
    abCount.Text               = "0 brainrots found"
    abCount.TextColor3         = AC.textDim
    abCount.Font               = Enum.Font.Gotham
    abCount.TextSize           = 10
    abCount.TextXAlignment     = Enum.TextXAlignment.Left
    abCount.Parent             = abPanel

    -- Scroll list
    local abScroll = Instance.new("ScrollingFrame")
    abScroll.Size                   = UDim2.new(1, 0, 1, -72)
    abScroll.Position               = UDim2.new(0, 0, 0, 68)
    abScroll.BackgroundTransparency = 1
    abScroll.BorderSizePixel        = 0
    abScroll.ScrollBarThickness     = 3
    abScroll.ScrollBarImageColor3   = AC.border
    abScroll.CanvasSize             = UDim2.new(0,0,0,0)
    abScroll.Parent                 = abPanel
    local abList = Instance.new("UIListLayout", abScroll)
    abList.SortOrder = Enum.SortOrder.LayoutOrder
    abList.Padding   = UDim.new(0, 2)
    abPad(6, 4, abScroll)

    -- ── TOGGLE BUTTON (floats top-left of screen) ─────────────────────
    local abToggleBtn = Instance.new("TextButton")
    abToggleBtn.Size             = UDim2.new(0, 110, 0, 30)
    abToggleBtn.Position         = UDim2.new(0, 16, 0, 16)
    abToggleBtn.BackgroundColor3 = AC.surface
    abToggleBtn.BorderSizePixel  = 0
    abToggleBtn.Text             = "● AUTO BUY"
    abToggleBtn.TextColor3       = AC.red
    abToggleBtn.Font             = Enum.Font.GothamBold
    abToggleBtn.TextSize         = 11
    abToggleBtn.AutoButtonColor  = false
    abToggleBtn.Active           = true
    abToggleBtn.Parent           = abSg
    abCorner(8, abToggleBtn)
    abStroke(1, AC.border, abToggleBtn)
    abDrag(abToggleBtn, abToggleBtn)

    -- ── MODULES ──────────────────────────────────────────────────────
    local abRS = game:GetService("ReplicatedStorage")
    local abDatas  = abRS:FindFirstChild("Datas")
    local abShared = abRS:FindFirstChild("Shared")
    local abUtils  = abRS:FindFirstChild("Utils")
    if not abDatas or not abShared or not abUtils then
        warn("[FlashHub AutoBuy] Required modules not found"); return
    end
    local abAnimData   = require(abDatas:WaitForChild("Animals"))
    local abAnimShared = require(abShared:WaitForChild("Animals"))
    local abNumUtils   = require(abUtils:WaitForChild("NumberUtils"))

    local abKN, abKNL = {}, {}
    for n, inf in pairs(abAnimData) do
        abKN[n] = n; abKNL[n:lower()] = n
        if inf.DisplayName then abKN[inf.DisplayName] = n; abKNL[inf.DisplayName:lower()] = n end
    end

    -- ── STATE ─────────────────────────────────────────────────────────
    local abEnabled       = false
    local abTarget        = nil
    local abManualTarget  = nil
    local abPlatform      = nil
    local abFF            = nil
    local abConns         = {}
    local abWatchConns    = {}
    local abPickerRows    = {}
    local abPickerConn    = nil

    -- ── CHARACTER HELPERS ─────────────────────────────────────────────
    local function abGetChar()
    ...
    local function abModelRoot(m)
        return m.PrimaryPart or m:FindFirstChild("HumanoidRootPart")
            or m:FindFirstChild("Head") or m:FindFirstChild("Root")
            or m:FindFirstChild("Torso") or m:FindFirstChildWhichIsA("BasePart", true)
    end

    -- ── IDENTIFY ──────────────────────────────────────────────────────
    local function abIdentify(m)
        if abKN[m.Name]          then return abKN[m.Name] end
        if abKNL[m.Name:lower()] then return abKNL[m.Name:lower()] end
        for _, attr in ipairs({"Index","AnimalIndex","Type","AnimalType","ID","Name"}) do
            local v = m:GetAttribute(attr)
            if v and type(v) == "string" then
                if abKN[v]          then return abKN[v] end
                if abKNL[v:lower()] then return abKNL[v:lower()] end
            end
        end
        for _, ch in ipairs(m:GetChildren()) do
            if ch:IsA("StringValue") and
               (ch.Name == "AnimalName" or ch.Name == "Type" or ch.Name == "AnimalType" or ch.Name == "Index") then
                local v = ch.Value
                if abKN[v]          then return abKN[v] end
                if abKNL[v:lower()] then return abKNL[v:lower()] end
            end
    ...
    -- ── WORLD SCAN ────────────────────────────────────────────────────
    local function abScan()
        local out, seen = {}, {}
        local plots = Workspace:FindFirstChild("Plots")
        local function tryModel(m)
            if seen[m] then return end; seen[m] = true
            if not m:IsA("Model") then return end
            if plots and m:IsDescendantOf(plots) then return end
            local int = abIdentify(m); if not int then return end
            local rt  = abModelRoot(m); if not rt then return end
            local mut = m:GetAttribute("Mutation") or "None"; if mut == "" then mut = "None" end
            local traits = {}
            local ta = m:GetAttribute("Traits"); if type(ta) == "table" then traits = ta end
            local ok, g = pcall(function() return abAnimShared:GetGeneration(int, mut, traits, nil) end)
            g = (ok and g) or 0
            local inf = abAnimData[int]
            table.insert(out, {
                model   = m, root = rt,
                name    = (inf and inf.DisplayName) or m.Name,
                gen     = g,
                genText = "$" .. abNumUtils:ToString(g) .. "/s",
            })
        end
        for _, fn in ipairs({"Animals","SpawnedAnimals","WildAnimals","Carpet","Entities"}) do
            local f = Workspace:FindFirstChild(fn)
            if f then
                for _, c in ipairs(f:GetChildren()) do
                    pcall(tryModel, c)
                    if c:IsA("Folder") or c:IsA("Model") then
                        for _, cc in ipairs(c:GetChildren()) do pcall(tryModel, cc) end
                    end
                end
            end
        end
        local skip = {Plots=true,Map=true,Terrain=true,Camera=true,Sky=true,Lighting=true}
        for _, c in ipairs(Workspace:GetChildren()) do
            if c:IsA("Model") and not skip[c.Name] and not c:FindFirstChildWhichIsA("Humanoid") then
                pcall(tryModel, c)
                for _, cc in ipairs(c:GetChildren()) do pcall(tryModel, cc) end
            end
        end
        table.sort(out, function(a, b) return a.gen > b.gen end)
        return out
    end

    -- ── PROMPT FIRING ─────────────────────────────────────────────────
    local function abFirePrompt(p)
        pcall(function()
            local savedHold = p.HoldDuration
            local savedDist = p.MaxActivationDistance
            p.HoldDuration = 0; p.MaxActivationDistance = AB_REACH
            if fireproximityprompt then pcall(fireproximityprompt, p) end
            pcall(function()
                if getconnections then
                    for _, c in ipairs(getconnections(p.Triggered)) do
            ...
            pcall(function()
                if getconnections then
                    for _, c in ipairs(getconnections(p.PromptButtonHoldBegan)) do
                        if type(c.Function) == "function" then pcall(c.Function) end
                    end
                    for _, c in ipairs(getconnections(p.PromptButtonHoldEnded)) do
                        if type(c.Function) == "function" then pcall(c.Function) end
                    end
                end
            end)
            task.defer(function()
                pcall(function() p.HoldDuration = savedHold end)
                pcall(function() p.MaxActivationDistance = savedDist end)
            end)
        end)
    end
    local function abFireAll(model)
        if not model or not model.Parent then return end
        pcall(function()
            for _, d in ipairs(model:GetDescendants()) do
                if d:IsA("ProximityPrompt") then abFirePrompt(d) end
            end
        end)
    end

    -- ── WATCH TARGET ──────────────────────────────────────────────────
    local function abWatch(m)
        for _, c in ipairs(abWatchConns) do pcall(function() c:Disconnect() end) end
        abWatchConns = {}
        if not m then return end
        abFireAll(m)
        table.insert(abWatchConns, m.DescendantAdded:Connect(function(d)
            if d:IsA("ProximityPrompt") then task.defer(function() abFirePrompt(d) end) end
        end))
    end

    -- ── PLATFORM / FF ─────────────────────────────────────────────────
    local function abEnsurePlat(pos)
        if not abPlatform or not abPlatform.Parent then
            local p = Instance.new("Part")
            p.Name = "ABPlat"; p.Size = Vector3.new(14,1,14)
            p.Anchored = true; p.CanCollide = true
            p.Transparency = 1; p.CanTouch = false
            p.Parent = Workspace; abPlatform = p
        end
        abPlatform.Position = Vector3.new(pos.X, pos.Y - 0.5, pos.Z)
    end
    local function abEnsureFF()
        local ch = LocalPlayer.Character; if not ch then return end
        if abFF and abFF.Parent == ch then return end
        pcall(function()
            if abFF then abFF:Destroy() end
            local ff = Instance.new("ForceField"); ff.Visible = false; ff.Parent = ch; abFF = ff
        end)
    end
    local function abProtect()
        pcall(function()
            local _, _, h = abGetChar(); if not h then return end
            h.Health = h.MaxHealth
            h:SetStateEnabled(Enum.HumanoidStateType.Dead, false)
            h.BreakJointsOnDeath = false
        end)
    end
    local function abEquipCarpet()
        pcall(function()
            local ch, _, hm = abGetChar(); if not ch or not hm then return end
            local c = LocalPlayer.Backpack:FindFirstChild("Flying Carpet") or ch:FindFirstChild("Flying Carpet")
            if c then hm:EquipTool(c) end
        end)
    end
    local function abGetTargetPos()
        if not abTarget or not abTarget.model or not abTarget.model.Parent then return nil end
        local r = abModelRoot(abTarget.model)
        if not r or not r.Parent then return nil end
        return r.Position
    end

    -- ── LOCK ON ───────────────────────────────────────────────────────
    local function abLockOn(item)
        abTarget = item
        abWatch(item.model)
        local r = abModelRoot(item.model)
        if r then
            abEnsurePlat(r.Position); abEnsureFF(); abProtect()
            local _, hrp = abGetChar()
            if hrp then
                local goal = Vector3.new(r.Position.X, r.Position.Y + AB_HEIGHT, r.Position.Z)
                if (hrp.Position - goal).Magnitude > AB_SNAP_DIST then
                    pcall(function()
                        hrp.AssemblyLinearVelocity = Vector3.zero
    ...
    -- ── PICKER UI ─────────────────────────────────────────────────────
    local function abUpdatePicker(list)
        for _, r in ipairs(abPickerRows) do pcall(function() r:Destroy() end) end
        abPickerRows = {}
        abCount.Text = #list > 0 and (#list .. " brainrots found") or "none nearby"
        if #list == 0 then
            abScroll.CanvasSize = UDim2.new(0,0,0,0)
            local empty = Instance.new("TextLabel")
            empty.Size = UDim2.new(1,0,0,44); empty.BackgroundTransparency = 1
            empty.Text = "No brainrots nearby..."; empty.TextColor3 = AC.textDim
            empty.Font = Enum.Font.Gotham; empty.TextSize = 11; empty.LayoutOrder = 1
            empty.Parent = abScroll; table.insert(abPickerRows, empty); return
        end
        for i, item in ipairs(list) do
            local isTarget = abTarget and abTarget.model == item.model
            local row = Instance.new("TextButton")
            row.Name = "Row_"..i; row.Size = UDim2.new(1,0,0,44)
            row.BackgroundColor3 = isTarget and AC.rowActive or AC.surface
            row.BorderSizePixel = 0; row.AutoButtonColor = false
            row.LayoutOrder = i; row.Text = ""; row.Parent = abScroll
            abCorner(7, row)
            if isTarget then abStroke(1, AC.accentDim, row) end

            if isTarget then
                local bar = Instance.new("Frame")
                bar.Size = UDim2.new(0,3,1,-12); bar.Position = UDim2.new(0,0,0,6)
                bar.BackgroundColor3 = AC.accent; bar.BorderSizePixel = 0; bar.Parent = row
                abCorner(2, bar)
            end

            local pill = Instance.new("Frame")
            pill.Size = UDim2.new(0,22,0,22); pill.Position = UDim2.new(0,10,0.5,-11)
            pill.BackgroundColor3 = i==1 and AC.gold or AC.bg
            pill.BorderSizePixel = 0; pill.Parent = row; abCorner(5, pill)
            local pillTxt = Instance.new("TextLabel")
            pillTxt.Size = UDim2.new(1,0,1,0); pillTxt.BackgroundTransparency = 1
            pillTxt.Text = tostring(i)
            pillTxt.TextColor3 = i==1 and Color3.fromRGB(30,20,0) or AC.textDim
            pillTxt.Font = Enum.Font.GothamBold; pillTxt.TextSize = 10; pillTxt.Parent = pill

            local nameLbl = Instance.new("TextLabel")
            nameLbl.Size = UDim2.new(1,-104,0,20); nameLbl.Position = UDim2.new(0,38,0,6)
            nameLbl.BackgroundTransparency = 1; nameLbl.Text = item.name
            nameLbl.TextColor3 = isTarget and AC.accent or AC.text
            ...
            genLbl.Size = UDim2.new(1,-104,0,14); genLbl.Position = UDim2.new(0,38,0,26)
            genLbl.BackgroundTransparency = 1; genLbl.Text = item.genText
            genLbl.TextColor3 = AC.accentDim; genLbl.Font = Enum.Font.Gotham
            genLbl.TextSize = 10; genLbl.TextXAlignment = Enum.TextXAlignment.Left; genLbl.Parent = row

            local badge = Instance.new("Frame")
            badge.Size = UDim2.new(0,54,0,22); badge.Position = UDim2.new(1,-62,0.5,-11)
            badge.BackgroundColor3 = isTarget and Color3.fromRGB(18,50,25) or AC.bg
            badge.BorderSizePixel = 0; badge.Parent = row; abCorner(5, badge)
            if isTarget then abStroke(1, AC.accentDim, badge) end
            local badgeTxt = Instance.new("TextLabel")
            badgeTxt.Size = UDim2.new(1,0,1,0); badgeTxt.BackgroundTransparency = 1
            badgeTxt.Font = Enum.Font.GothamBold; badgeTxt.TextSize = 9
            badgeTxt.Text = isTarget and "BUYING" or "SELECT"
            badgeTxt.TextColor3 = isTarget and AC.accent or AC.textDim; badgeTxt.Parent = badge

            row.MouseEnter:Connect(function()
                if not isTarget then row.BackgroundColor3 = AC.rowHover; badgeTxt.TextColor3 = AC.text end
            end)
            row.MouseLeave:Connect(function()
                if not isTarget then row.BackgroundColor3 = AC.surface; badgeTxt.TextColor3 = AC.textDim end
            end)
            local cap = item
            row.MouseButton1Click:Connect(function()
                abManualTarget = cap; abLockOn(cap)
            end)
            table.insert(abPickerRows, row)
        end
        abScroll.CanvasSize = UDim2.new(0,0,0,#list*46+8)
    end

    -- ── PICKER LOOP ───────────────────────────────────────────────────
    local function abStartPickerLoop()
        if abPickerConn then abPickerConn:Disconnect() end
        local t = 0
        abPickerConn = RunService.Heartbeat:Connect(function(dt)
            if not abEnabled then return end
            t = t + dt; if t < AB_SCAN_INT then return end; t = 0
            task.spawn(function() abUpdatePicker(abScan()) end)
        end)
    end
    local function abStopPickerLoop()
        if abPickerConn then abPickerConn:Disconnect(); abPickerConn = nil end
        for _, r in ipairs(abPickerRows) do pcall(function() r:Destroy() end) end
        abPickerRows = {}; abCount.Text = "0 brainrots found"
        abScroll.CanvasSize = UDim2.new(0,0,0,0)
    end

    -- ── STOP / START ──────────────────────────────────────────────────
    local function abStop()
        abEnabled = false; abManualTarget = nil
        for _, c in ipairs(abConns)      do pcall(function() c:Disconnect() end) end
        for _, c in ipairs(abWatchConns) do pcall(function() c:Disconnect() end) end
        abConns = {}; abWatchConns = {}
        if abPlatform and abPlatform.Parent then abPlatform:Destroy() end
        if abFF       and abFF.Parent       then abFF:Destroy()       end
        abPlatform = nil; abTarget = nil; abFF = nil
        abStopPickerLoop()
        abPanel.Visible = false
        abDot.Text = "OFF"; abDot.TextColor3 = AC.red
        abToggleBtn.TextColor3 = AC.red
    end

    local function abStart()
        abStop(); abEnabled = true
        abPanel.Visible = true
        abDot.Text = "ON"; abDot.TextColor3 = AC.accent
        abToggleBtn.TextColor3 = AC.accent
        abEquipCarpet()
        local list = abScan(); abUpdatePicker(list)
        if not abManualTarget and #list > 0 then abLockOn(list[1]) end
        abStartPickerLoop()

        local lastFire = 0
        table.insert(abConns, RunService.Heartbeat:Connect(function(dt)
            if not abEnabled then return end
            abProtect(); abEnsureFF()
            local tp = abGetTargetPos(); if not tp then return end
            local _, hrp = abGetChar(); if not hrp then return end
            abEnsurePlat(tp)
            local goal = Vector3.new(tp.X, tp.Y + AB_HEIGHT, tp.Z)
            local diff = hrp.Position - goal
            local dist = diff.Magnitude
            if dist > AB_SNAP_DIST then
                pcall(function()
                    hrp.AssemblyLinearVelocity = Vector3.zero
                    hrp.CFrame = CFrame.new(goal)
                end)
            elseif dist > 0.15 then
                pcall(function()
                    local dir = diff.Unit
                    local speed = math.min(AB_SMOOTH_SPD, dist * 12)
                    local vel = hrp.AssemblyLinearVelocity
                    hrp.AssemblyLinearVelocity = Vector3.new(-dir.X*speed, -vel.Y*0.6, -dir.Z*speed)
                end)
            else
                pcall(function()
                    local vel = hrp.AssemblyLinearVelocity
                    hrp.AssemblyLinearVelocity = Vector3.new(vel.X*0.3, -vel.Y*0.5, vel.Z*0.3)
                end)
            end
            local now = os.clock()
            if now - lastFire >= AB_FIRE_RATE then
                lastFire = now
                abFireAll(abTarget and abTarget.model)
            end
        end))
        ...
        task.spawn(function()
            while abEnabled do
                abFireAll(abTarget and abTarget.model)
                task.wait(AB_FIRE_RATE * 2)
            end
        end)

        task.spawn(function()
            while abEnabled do
                local mt = abManualTarget
                if mt and (not mt.model or not mt.model.Parent) then abManualTarget = nil end
                local ct = abTarget
                if not ct or not ct.model or not ct.model.Parent then
                    if abPlatform and abPlatform.Parent then abPlatform:Destroy(); abPlatform = nil end
                    if abFF       and abFF.Parent       then abFF:Destroy();       abFF       = nil end
                    abTarget = nil
                    local newMt = abManualTarget
                    if newMt and newMt.model and newMt.model.Parent then
                        abEquipCarpet(); abLockOn(newMt)
                    else
                        local l = abScan()
                        if #l > 0 then abEquipCarpet(); abLockOn(l[1]) end
                    end
                else
                    local newMt = abManualTarget
                    if newMt and newMt.model and newMt.model.Parent and ct.model ~= newMt.model then
                        abLockOn(newMt)
                    end
                end
                task.wait(1)
    ...
    abToggleBtn.MouseButton1Click:Connect(function()
        if abEnabled then abStop() else task.spawn(abStart) end
    end)
    -- expose toggle so keybind and settings toggle can call it
    _G._autoBuyToggle = function()
        if abEnabled then abStop() else task.spawn(abStart) end
    end
    _G._autoBuyActive = function() return abEnabled end
end)
